...
In the residuals vs. fitted plot, the differences between observed and predicted values (aka the residuals) are plotted on the y-axis and the predicted values(fitted values) are plotted on the x-axis. Besides the two clusters (which makes sense because we are using categorical data and we have two categories), it seems like the points are scattered, indicating that there may be no non-linearity or heteroskedasticity.

In the normal QQ plot, we are comparing the distribution of residuals to the normal distribution. The points here generally follow a straight line but deviate slightly at the edges, indicating that there may be some deviation from normality in the residuals.

In the scale-location plot, the square root of the standardized residuals are plotted on the y axis, and the fitted values are plotted on the x-axis. The spread seems about constant, so we can likely assume homoskedasticity.

In the leverage plot, the standardized residuals are plotted on the y axis and category is plotted on the x axis. Points with high leverage are indicated (observations 25, 77, and 258)???
...
This is paired data, so the results will depend on the subject. 
...
The estimates, t-statistic, and p-value are equivalent to the lm summary.
```{r}
# T-test
...
The results are the same as the paired t test.
```{r}
...
Conclusion: Brain activity in the thalamus measured using the multiband 4 with 2 mm resolution is NOT the same as brain activity measured using the single band with 3.3 mm resolution.
...
Age and sex were not estimated because they are correlated with the subjects (since the age and sex info comes from the subject). Therefore, there is multicollinearity. 

Mathematically, the columns of the design matrix will be populated with the subjects. So column 2 is subject 1, and column 3 is subject 2, and so on.
...
Yes, multicollinearity is an issue. There are some very large VIFs.
```{r}
library(car)
...
No VIFs alone are greater than 10 (the rule of thumb), but some VIFs are still somewhat high, like Jitter_Percent and NHR,  which are both >4.
```{r}
...
In the leverage plot, the standardized residuals are plotted on the y axis and leverage is plotted on the x axis. Points with high leverage are indicated???????????????????????????????????????????
```{r}
...
The coefficient of NHR changes by around 72%. Thus, there is evidence of omitted variable bias, since age may have been a confounder that affected the relationship between NHR and motor_UPDRS.
```{r}
# model without 'age'
...
# pct change
percent_change <- ((beta_no_age - beta_crude) / beta_crude) * 100
percent_change
```
...
Yes, age modifies the relationship between NHR and motor_UPDRS in a way where if age increases, the effect of NHR on motor_UPDRS decreases. The interaction coefficient is statistically significant at alpha = 0.05.
```{r}
...
The change in expected motor_UPDRS for a one-unit increase in NHR, holding all other variables constant, and age being 1, is 134.66364-1.96831 = 132.69533.??????????????????????????????????????????????????????????????????????????
...
The change in expected motor_UPDRS for a one-unit increase in NHR, holding all other variables constant, and age being the average age, is 7.10765-1.96831 = 5.13934.
```{r}
# age centered
