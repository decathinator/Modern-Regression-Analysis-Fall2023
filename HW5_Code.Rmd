...
$$y_i = \beta_0 + \beta_1DX + \beta_2PTGENDER + \beta_3APOE4_1 + \beta_4APOE4_2 + g_1(AGEVISIT) + g_2(PTEDUCAT) + \epsilon$$
...
The term has fewer unique covariate combinations than specified maximum degrees of freedom.
...
Black curve in 1.9 is $\hat{y_i} = \hat{\beta_0} + \hat{\beta_1}DX + \hat{\beta_2}PTGENDER + \hat{\beta_3}APOE4_1 + \hat{\beta_4}APOE4_2 + \hat{g_1}(AGEVISIT) + \hat{g_2}(PTEDUCAT)$

Black curve in 1.8 is $g_1(AGEVISIT)$.
...
1.9: $Var [\hat{\beta_0} + \hat{\beta_1}DX + \hat{\beta_2}PTGENDER + \hat{\beta_3}APOE4_1 + \hat{\beta_4}APOE4_2 + \hat{g_1}(AGEVISIT) + \hat{g_2}(PTEDUCAT)]$

1.8: $Var[\hat{g_1}(AGEVISIT)] $
...
```{r}
itsadug::plot_smooth(q2, view="AGE_VISIT")
itsadug::plot_smooth(q12, view="AGE_VISIT", add=TRUE)
```
...
```{r}
itsadug::plot_smooth(q14, view="AGE_VISIT")
itsadug::plot_smooth(q2, view="AGE_VISIT")
...
The approximate p-value is <2e-16. Hippocampal volume tends to decrease with `AGE_VISIT`.
...
```{r}
q20 <- gam(HIPPOCAMPUS_VOL~s(AGE_VISIT)+s(PTEDUCAT)+DX+PTGENDER+APOE4 + s(AGE_VISIT, by = APOE4), data= mcidata_cx, method="REML")
summary(q20)
```
